...
#ifndef __R_FPLS_SB_H__
#define __R_FPLS_SB_H__
...
template<typename RegularizationType> class R_FPLS_SB {
  public:
    using ModelType = std::decay_t<FPLS_SB<RegularizationType>>;
    using ModelBaseType = std::decay_t<FPLS_BASE<RegularizationType, ModelType>>;
    using SolverType = RSVDType<ModelBaseType>;
    // constructor
    R_FPLS_SB() : calibration_strategy_(Calibration::off) {}
    // getters
...
class R_FPLS_SB_SpaceOnly : public R_FPLS_SB<SpaceOnly> {
  public:
    R_FPLS_SB_SpaceOnly(Rcpp::Environment pde,
                      int sampling_type,
